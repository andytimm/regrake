...
#| output: true
n_pew <- nrow(pew)
regrake_weights_1 <- regrake_result_1$weights * n_pew
...
regrake_1 <- tibble(value = regrake_weights_1, method = "regrake")
...
# Main effects: exact matching
# Interactions: L2 (soft) matching
...
## Results with mixed constraints

```{r}
#| echo: true
#| output: true
regrake_weights_2 <- regrake_result_2$weights * n_pew

# Compare weight distributions
raked_2 <- enframe(weights(pew_raked_2) * n_pew) %>% mutate(method = "rake")
regrake_2 <- tibble(value = regrake_weights_2, method = "regrake")
