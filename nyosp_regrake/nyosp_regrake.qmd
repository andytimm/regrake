...
#| output: true
#| fig-height: 4
# Show how different lambda values affect weight distribution
lambdas <- c(0.1, 1, 10, 100)
lambda_results <- lapply(lambdas, function(lam) {
  result <- regrake(
    data = pew,
    formula = ~ rr_l2(recode_age_bucket) + rr_l2(recode_female) +
                rr_l2(recode_race) + rr_l2(recode_region),
    population_data = cces_wt_design,
    pop_type = "survey_design",
    regularizer = "entropy",
    lambda = lam
  )
  tibble(weight = result$weights, lambda = paste0("λ = ", lam))  # Already sums to n
})
